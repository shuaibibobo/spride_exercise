...
class Product(threading.Thread):
    ...
    def get_follows(self,userid,totle):
        try:
            res = requests.get(COMMENT.format(userid,totle),headers=HEADERS,timeout=20)
            data = res.json()
...
class Consumer(threading.Thread):
    ...
    def get_info(self,key):
        try:
            res = requests.get(START_URL.format(key),headers=HEADERS,timeout=20)
