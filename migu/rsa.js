window=this;
navigator=this;
var alert = new Function('message', 'console.log("ALERT: " + message)');
alert.prototype.version = '1.0';
...
function getPwd(account,pwd){
    c = new RSAKey;
    c.setPublic(a.result.modulus, a.result.publicExponent);
    var pwd = c.encrypt(pwd);
    var account=c.encrypt(account);
    return [account,pwd];
}
console.log(getPwd("asfa","af"))
function getFingerprint() {

    var c ='"{"user_agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Sa","language":"zh-CN","color_depth":"24","pixel_ratio":"1.25","hardware_concurrency":"12","resolution":"1536,864","available_resolution":"1536,824","timezone_offset":"-480","session_storage":"1","local_storage":"1","indexed_db":"1","open_database":"1","cpu_class":"unknown","navigator_platform":"Win32","do_not_track":"unknown","regular_plugins":"Chrome PDF Plugin::Portable Document Format::application/x-google-chrome-pdf~pdf,Chrome PDF Viewer::","webgl_vendor":"Google Inc. (Intel)~ANGLE (Intel, Intel(R) UHD Graphics 630 Direct3D11 vs_5_0 ps_5_0, D3D11)","adblock":"false","has_lied_languages":"false","has_lied_resolution":"false","has_lied_os":"true","has_lied_browser":"false","touch_support":"20,false,false","js_fonts":"Arial,Arial Black,Arial Narrow,Book Antiqua,Bookman Old Style,Calibri,Cambria,Cambria Math,Century,C"}"'
        , d ='23e9dff8041bf02e3b926de5013fe63e'
        , e = c.length
        , f = ""
        , g = new RSAKey;
...
// console.log(getFingerprint(a.result.modulus,a.result.publicExponent));
...
function rsaFingerprint(a, b, details, result) {		//details和result是固定值，直接作为参数传进来赋值
    var c = details
        , d = result
        , e = c.length
        , f = ""
    ...
    // }
    return {
        fingerPrintDetail: f,	//这里代码会返回字典类型的数据，直接可以合并到表单里，所以直接为修改表单里可以用key值
        fingerPrint: h
    }
}
